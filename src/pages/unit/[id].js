...
export default function ChapterDetail() {
    ...
    const { title, chapter } = router.query;
    ...
    useEffect(() => {
        ...
        console.log("useEffect triggered with title:", title);
    
        fetch(`/api/getResource?title=${encodeURIComponent(title)}`)
            .then((res) => res.json())
            .then((data) => {
            ...
            {course ? (
                <Unit 
                    ...
                    onSelectOption={(option) => {
                        router.push(`/unit/${encodeURIComponent(option.title)}`, undefined, { shallow: true }); 
